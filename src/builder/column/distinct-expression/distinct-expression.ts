...
export class ParcelQLDistinctExpressionBuilder 
    extends BaseColumnBuilder<ParcelQLDistinctExpression> {
    ...
    protected beforeValidatingQuery(knex: Knex<any, any[]>): void {
        ...
        for (const col of this.query.distinct) {
            if (typeof col === 'string') {
                this.cols.push(new this.parent({column: col}, this.parent, 
                    [ParcelQLValueExpressionBuilder.name]));
            } else {
                this.cols.push(new this.parent(col, this.parent, [ParcelQLValueExpressionBuilder.name]));
            }
        }
    }
    ...
    protected _validateQuery(): void {
        if (this.isQuerySchemaSupported()) {
            if (this.query.distinct.length === 0) {
                throw new ParcelQLValidationError(`Distinct expression must have at least one column`);
            }
        } else {
            throw new ParcelQLValidationError(`Distinct expression must have distinct property`);
        }
    }
