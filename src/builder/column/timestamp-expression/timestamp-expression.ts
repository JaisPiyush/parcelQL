...
export class ParcelQLTimestampExpressionBuilder extends BaseQueryBuilder<ParcelQLTimestampExpression> {
    protected _validateQuery(): void {
        ...
        if (
            Array.isArray(this.query.parameters) &&
            this.query.parameters.length > 1
        ) {
            throw new ParcelQLValidationError(
    ...
    protected _build(knex: Knex): Knex.Raw {
        const parameters: string[] = Array.isArray(this.query.parameters)
            ? this.query.parameters
            : [this.query.parameters];

        if (this.query.name.includes('tz')) {
            return knex.raw(
                `${this.query.name.replace('tz', '')} ? with time zone`,
                parameters
            );
        }
