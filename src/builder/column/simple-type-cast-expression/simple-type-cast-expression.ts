...
import { ParcelQLSimpleColumnExpressionWithType, ParcelQLSupportedTypeCast } from "../../../schema/column-expression/base-column-expression";
import { parcelQLSupportedTypeCast } from "../../../schema/constants";
import { BaseQueryBuilder } from "../../base-query-builder";

export class ParcelQLTypeCastExpression 
 extends BaseQueryBuilder<ParcelQLSimpleColumnExpressionWithType['type']> {
    ...
    protected _isProvisionedQuery(): void {
        ...
        if (this.query) {
            ...
            types.forEach(element => {
                if (!parcelQLSupportedTypeCast.includes(element as ParcelQLSupportedTypeCast)) {
    ...
    protected _build(knex: Knex<any, any[]>): Knex.Raw<any> {
        ...
        const types = Array.isArray(this.query)
        ? this.query
        : [this.query];
        let query = '';
